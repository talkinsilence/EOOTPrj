...
SELECT * FROM 
(SELECT M.NAME, M.PROFILEPIC, F.* ,(ROW_NUMBER() OVER (ORDER BY F.REGDATE DESC)) NUM 
FROM MEMBERS M INNER JOIN FAMPOSTS F ON M.MID=F.WRITER 
WHERE (F.TITLE LIKE '%f%') OR (F.CONTENT LIKE '%f%')) MJOINF 
WHERE MJOINF.NUM BETWEEN 1 AND 30;


DELETE FROM FAMPOSTS, FAMPOSTFILES, FAMPOSTCOMMENTS WHERE FAMPOSTS.CODE = '2' AND FAMPOSTFILES.FAMPOSTCODE ='2' AND FAMPOSTCOMMENTS.FAMPOSTCODE = '2';

delete famposts.title from famposts where code='10';

SELECT TOP 1 code FROM FAMPOSTS WHERE REGDATE < (SELECT REGDATE FROM FAMPOSTS
WHERE CODE = '5') ORDER BY REGDATE DESC;


SELECT M.NAME, M.PROFILEPIC, F.*, FPF.SRC, 
(ROW_NUMBER() OVER (ORDER BY F.REGDATE DESC)) NUM 
FROM MEMBERS M INNER JOIN FAMPOSTS F ON F.WRITER = M.MID 
LEFT OUTER JOIN FAMPOSTFILES FPF ON F.CODE = FPF.FAMPOSTCODE WHERE F.CODE = (SELECT TOP 1 code FROM FAMPOSTS WHERE REGDATE > (SELECT REGDATE FROM FAMPOSTS WHERE CODE = '5') ORDER BY REGDATE ASC)

SELECT FC.*, M.NAME, M.PROFILEPIC, (ROW_NUMBER() OVER (ORDER BY FC.REGDATE DESC)) NUM FROM FAMPOSTCOMMENTS FC INNER JOIN MEMBERS M ON FC.WRITER = M.MID WHERE FC.FAMPOSTCODE = (SELECT TOP 1 code FROM FAMPOSTCOMMENTS WHERE REGDATE < (SELECT REGDATE FROM FAMPOSTCOMMENTS WHERE CODE = '5') ORDER BY REGDATE DESC)

INSERT INTO fampostfiles VALUES('1', 'fampostImage1.jpg','1', 'jpg',getdate());
...
INSERT INTO FAMPOSTS(CODE, TITLE, CONTENT, WRITER, REGDATE, HIT, LIKECNT, CLIPCNT, COMMENTCNT, SRC) 
VALUES(1, '배고픔', '먹을거 없나?', 'nami@eoot.com', getDate(), 0, 0, 0, 0, 'src');

INSERT INTO FAMPOSTFILES(CODE, SRC, FAMPOSTCODE, TYPE, REGDATE) 
VALUES(1, 'src', '2', '3', getDate());

INSERT INTO FAMPOSTCOMMENTS(CODE, CONTENT, WRITER, FAMPOSTCODE, REGDATE) 
VALUES(, 'hi', 'viovio@eoot.com', '3', getDate());
