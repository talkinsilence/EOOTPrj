...
public interface FamPostCommentDao {
	...
	@Select("SELECT FC.*, M.NAME, M.PROFILEPIC, (ROW_NUMBER() OVER (ORDER BY FC.REGDATE DESC)) "
			+ "NUM FROM FAMPOSTCOMMENTS FC INNER JOIN MEMBERS M "
			+ "ON FC.WRITER = M.MID WHERE FC.FAMPOSTCODE = "
			+ "(SELECT TOP 1 code FROM FAMPOSTCOMMENTS "
			+ "WHERE REGDATE < (SELECT REGDATE FROM FAMPOSTCOMMENTS WHERE CODE = #{curCode}) "
